...
export default function WorkoutExerciseSelectModal({ isOpen, onClose, onSelectComplete, userGymId }) {
  ...
  const fetchExercises = async () => {
    ...
    try {
      ...
      if (userGymId && (activeCategory === '머신' || activeCategory === '웨이트')) {
        // 점포 기구 우선 로드 (Join)
        const { data: gymEqData, error: geError } = await supabase
          .from('gym_equipments')
          .select('*, equipments(*)')
          .eq('gym_id', userGymId);
        ...
        masterData = gymEqData
          .filter(ge => ge.equipments)
          .map(ge => ge.equipments)
          .filter(e => e.category === activeCategory);

        if (activeSubCategory && activeCategory !== '유산소') {
          masterData = masterData.filter(e => e.sub_category === activeSubCategory);
        }
      } else {
        // 일반 마스터 로드
      ...
      const mData = masterData.map(d => ({ ...d, isCustom: false }));

      // 필터링된 커스텀 데이터와 마스터 데이터 합치기
      setExercises([
        ...mData,
        ...cData.filter(c => c.category === activeCategory && c.sub_category === activeSubCategory)
      ]);
    } catch (err) {
      console.error(err);
      setErrorMsg(err.message || '운동 데이터를 불러오는데 실패했습니다.');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div className="flex space-x-2 overflow-x-auto scrollbar-hide py-1">
                {CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`shrink-0 px-4 py-2 rounded-full text-[14px] font-bold transition-colors ${activeCategory === cat ? 'bg-[#191f28] text-white shadow-md' : 'bg-white border border-[#e5e8eb] text-[#4e5968] hover:bg-gray-50'
                      }`}
                  >
                    {cat}
                  </button>
              ...
              <div className="flex space-x-2 overflow-x-auto scrollbar-hide mt-3 py-1 pl-1">
                {SUB_CATEGORIES.map(sub => (
                  <button
                    ...
                    className={`shrink-0 px-3 py-1.5 rounded-lg text-[13px] font-bold transition-colors ${activeSubCategory === sub ? 'bg-[#e8f3ff] text-[#3182f6]' : 'bg-transparent text-[#8b95a1] hover:text-[#4e5968]'
                      }`}
                  >
                    {sub}
                  </button>
                  ...
                  {filteredExercises.map(ex => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-colors ${isSelected ? 'bg-[#e8f3ff]' : 'bg-white hover:bg-[#f9fafb]'
                          }`}
                      >
